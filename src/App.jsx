...
// ─── Constants ────────────────────────────────────────────────────────────────
const BANDWIDTH = {
  1: { label: "Overloaded", color: "#f43f5e" },
  2: { label: "Busy",       color: "#f97316" },
  3: { label: "Balanced",   color: "#eab308" },
  4: { label: "Available",  color: "#22c55e" },
  5: { label: "Open",       color: "#06b6d4" },
};
...
:root{
  --bg:#070810;--surface:#0e0f1c;--card:#13142a;--border:#1e2040;--border2:#252650;
  --text:#e2e4f0;--muted:#5a5c80;--accent:#5b5ff5;--accent2:#8b5cf6;--green:#22c55e;--red:#f43f5e;
}
...
.login-divider{font-size:11px;color:var(--muted);margin:20px 0;letter-spacing:0.1em;text-transform:uppercase}
    ...
    <div className="login-screen">
      ...
      <div className="login-card">
        ...
        <div className="login-sub">
          Your team's daily standup tracker.<br/>Sign in with your work Google account to continue.
        </div>
    ...
    <div className="access-denied">
      <div className="access-denied-card">
        ...
        <div className="access-denied-sub">
          Your account <strong>{user?.email}</strong> is not authorised to access this app.<br/><br/>
          Please sign in with your <strong>@{ALLOWED_DOMAIN}</strong> account.
        </div>
        <button className="btn-primary" onClick={onSignOut} style={{margin:"0 auto"}}>
          Sign out & try again
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  // Set selected member from logged-in user's first name
  useEffect(() => {
    if (!user) return;
  ...
  // Load all history once authenticated
  ...
  // Pre-fill form from today's existing entry
  useEffect(() => {
    ...
    if (todayEntry) {
      setForm({
        ...
        tasks:     todayEntry.tasks?.length ? todayEntry.tasks : [{text:"",status:"In Progress"}],
  ...
  const saveEntry = async () => {
    ...
    try {
      ...
      setHistory(prev => {
        const filtered = (prev[selectedMember]||[]).filter(e=>e.date!==TODAY);
        return { ...prev, [selectedMember]: [entry,...filtered].sort((a,b)=>b.date.localeCompare(a.date)) };
      });
  ...
  const generateAiSummary = async () => {
    ...
    try {
      const snapshot = TEAM_MEMBERS.map(m => {
        ...
        if (!latest||latest.date!==TODAY) return `${m}: No update today`;
        const tasks = latest.tasks?.map(t=>`${t.text} [${t.status}]`).join(", ")||"No tasks";
        return `${m}: Yesterday="${latest.yesterday||"—"}" Today="${latest.today||"—"}" Blockers="${latest.blockers||"None"}" Bandwidth="${BANDWIDTH[latest.bandwidth]?.label}" Tasks="${tasks}"`;
      }).join("\n");
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000, messages:[{ role:"user", content:`You are an engineering manager's assistant. Today is ${fmt(TODAY)}.\n\nTeam standup:\n${snapshot}\n\nReply ONLY in this exact JSON (no markdown):\n{"overview":"2-3 sentence team status","highlights":"Key wins (bullet points starting with •, newline-separated)","risks":"Blockers or concerns, or 'No blockers today ✓'","capacity":"Who is available vs stretched","actions":"Manager action items (starting with →, newline-separated)"}` }] })
      });
      const data = await res.json();
      const text = data.content?.map(b=>b.text||"").join("")||"";
      setAiSummary(JSON.parse(text.replace(/```json|```/g,"").trim()));
    } catch(e) {
      setAiSummary({overview:"Failed to generate summary. Please try again.",highlights:"",risks:"",capacity:"",actions:""});
    }
    setAiLoading(false);
  };

  // ── Derived stats ──
  const updatedToday = TEAM_MEMBERS.filter(m=>(history[m]||[]).find(e=>e.date===TODAY)).length;
  const blockedCount = TEAM_MEMBERS.filter(m=>{ const e=(history[m]||[]).find(x=>x.date===TODAY); return e&&e.blockers?.trim(); }).length;
  const avgBw = (() => {
    const bws = TEAM_MEMBERS.map(m=>{ const e=(history[m]||[]).find(x=>x.date===TODAY); return e?e.bandwidth:null; }).filter(Boolean);
  ...
  if (!user) return (
    <><style>{CSS}</style><LoginScreen onLogin={handleLogin} error={loginError} loading={loginLoading}/></>
  );

  const domain = user.email?.split("@")[1];
  if (domain !== ALLOWED_DOMAIN) return (
    <><style>{CSS}</style><AccessDenied user={user} onSignOut={handleSignOut}/></>
  );
        ...
        {view==="member" && (
          <div className="body two-col">
            ...
            <main className="main">
              ...
              <div className="history-sub">Update History <span style={{color:"var(--muted)",fontWeight:400}}>— {memberEntries.length} entries</span></div>
              {dataLoading ? (
                <div className="empty-state"><span className="spinner" style={{width:24,height:24}}/></div>
              ) : memberEntries.length===0 ? (
                <div className="empty-state"><div className="empty-icon">📋</div>No history yet.<br/>Submit your first update above.</div>
              ) : (
            ...
            <main className="main">
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24}}>
                <div>
                  ...
                  <div className="page-sub">Team snapshot for {fmt(TODAY)}</div>
                </div>
              ...
              <div className="stats-row">
                <div className="stat-card"><div className="stat-val" style={{color:"var(--accent)"}}>{updatedToday}</div><div className="stat-label">Updated today</div></div>
                <div className="stat-card"><div className="stat-val" style={{color:"#94a3b8"}}>{TEAM_MEMBERS.length-updatedToday}</div><div className="stat-label">Pending updates</div></div>
                <div className="stat-card"><div className="stat-val" style={{color:"var(--red)"}}>{blockedCount}</div><div className="stat-label">Blocked today</div></div>
                <div className="stat-card"><div className="stat-val" style={{color:"var(--green)",fontSize:20,paddingTop:4}}>{avgBw}</div><div className="stat-label">Avg bandwidth</div></div>
              </div>

              {aiSummary&&(
                <div className="ai-summary-box">
                  <div className="ai-summary-title">✦ AI Manager Briefing</div>
                  ...
                  return(
                    <div className="mgr-card" key={m}>
                      <div className="mgr-card-header">
                        ...
                        {hasToday?(
                          <span className="bw-badge" style={{background:BANDWIDTH[latest.bandwidth]?.color+"20",color:BANDWIDTH[latest.bandwidth]?.color}}>{BANDWIDTH[latest.bandwidth]?.label}</span>
                        ):(
                          <span className="no-update">No update today</span>
                        )}
                        <span className="mgr-meta">{latest?fmt(latest.date):"—"}</span>
                      </div>
                      {hasToday&&(
                        <>
                          <div className="mgr-body">
                            <div><div className="mgr-field-label">Yesterday</div><div className="mgr-field-text">{latest.yesterday||"—"}</div></div>
                            <div><div className="mgr-field-label">Today</div><div className="mgr-field-text">{latest.today||"—"}</div></div>
                            <div>
                              <div className="mgr-field-label" style={{color:latest.blockers?"var(--red)":"var(--muted)"}}>{latest.blockers?"⚑ Blockers":"Blockers"}</div>
                              <div className="mgr-field-text" style={{color:latest.blockers?"var(--red)":"#b0b2cc"}}>{latest.blockers||"None"}</div>
                            </div>
                          </div>
