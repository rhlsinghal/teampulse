        ...
        <div>
          ...
          {summary.daysSubmitted === 0 ? (
            ...
            <>
              ...
              <div className="card">
                <div className="card-header"><span className="card-title">Task breakdown</span><span className="card-meta">{summary.daysSubmitted} days · {summary.totalTasks} tasks</span></div>
                <table className="data-table">
                  <thead><tr><th>Date</th><th>Client</th><th>Task</th><th>Status</th></tr></thead>
                  <tbody>
                    {summary.entries.flatMap((e, ei) => (e.tasks||[]).filter(t=>t.text?.trim()).map((t, ti) => (
                      <tr key={`${ei}-${ti}`}>
                        <td style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "var(--muted)" }}>{fmt(e.date)}</td>
                        <td>{t.client ? <span className="badge badge-blue" style={{ fontSize: 11 }}>{t.client}</span> : <span style={{ color: "var(--faint)" }}>—</span>}</td>
                        <td className="text-sm">{t.text}</td>
                        <td>{(() => { const s = OUTCOME_STYLE[t.status]||OUTCOME_STYLE[t.outcome]; return s ? <span style={{ fontSize:11,padding:"2px 8px",borderRadius:20,fontWeight:500,background:s.bg,color:s.color,border:`0.5px solid ${s.bd}`}}>{t.status||t.outcome}</span> : <span style={{fontSize:11,color:"var(--faint)"}}>{t.status||"—"}</span>; })()}</td>
                      </tr>
                    )))}
                  </tbody>
                </table>
